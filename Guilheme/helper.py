...
#ler csv, tokeniza as opnioes e retorna tuplas
from sklearn.naive_bayes import MultinomialNB
from sklearn.svm import LinearSVC


def read_base(csv_file=str):
    base = []
    tknz = TweetTokenizer()
    with open(csv_file, 'r') as csv_file:
        ...
        for row in spam_reader:
            try:
                temp1 = tknz.tokenize(row[3])
                temp1 = clean_word(temp1)
                temp2 = int(row[0])
...
def clean_word(list):
    ...
    for word in list:
        ...
        if 'https' in word:
            continue
        word = removeracentoecharesp(word)
        word = removeemoji(word)
        result.append(word)
        frase = ' '.join(str(r) for r in result)
    return frase


def removeracentoecharesp(palavra):
